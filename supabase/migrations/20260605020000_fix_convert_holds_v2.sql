...
-- because the original RETURNS TABLE OUT column `id` collided with the `id`
-- columns of seat_holds / section_enrollments / meeting_enrollments in the body.
--
-- Renaming the OUT column changes the function's return type, which CREATE OR
-- REPLACE cannot do (SQLSTATE 42P13) — so DROP first, then CREATE. Re-issued at a
-- timestamp later than the newest applied migration (20260605010000) so db push
-- applies it in order (the earlier 20260605000000 attempt was skipped as
-- out-of-order). Idempotent: safe on a fresh replay.
