...
const BOARD_ID = "6980bdf1ce8e7c9dcd137dff";
...
function getRecentCommits() {
  ...
  return log
    ...
    .filter(
      (c) =>
        c.subject.toLowerCase().includes("cart") ||
        c.subject.toLowerCase().includes("register") ||
        c.subject.toLowerCase().includes("waitlist") ||
        c.subject.toLowerCase().includes("portal") ||
        c.subject.toLowerCase().includes("session") ||
        c.subject.toLowerCase().includes("profile") ||
        c.subject.toLowerCase().includes("checkout") ||
        c.subject.toLowerCase().includes("semester") ||
        c.body?.toLowerCase().includes("user-facing")
    );
}
...
registration platform. You are focused ONLY on the public-facing registration
portal — the experience families and parents use to browse sessions, manage
their cart, register dancers, and pay.

Given a list of git commits, generate Trello user stories for the user portal board.

Return ONLY valid JSON — no markdown, no explanation — in this format:
[
  {
    "name": "Short card title",
    "desc": "As a parent/family, [what was built/fixed] so that [outcome].",
    "list": "Done",
...
Omit trivial commits (version bumps, typo fixes, merge commits).`,
      messages: [
        {
          role: "user",
          content: `Here are this week's user-facing commits from the AYDT codebase:\n\n${commitText}`,
        },
      ],
    }),
  });
...
async function run() {
  console.log("Reading AYDT git history (user-facing commits)...");
  const commits = getRecentCommits();

  if (!commits || commits.length === 0) {
    console.log("No user-facing commits in the past 7 days. Nothing to sync.");
    return;
  }
